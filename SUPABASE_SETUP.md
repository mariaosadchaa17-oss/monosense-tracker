...
2. Відкрийте **SQL Editor** і послідовно виконайте:
   ...
   - `supabase/migrations/202607270010_active_households_and_atomic_import.sql`.
...
Для правил без автоматичного списання cron надсилає push та/або Telegram-
нагадування за 24 години до платежу. Після дати платежу правило автоматично
переноситься на наступний день, тиждень, місяць або рік.
...
Користувач вмикає push самостійно у налаштуваннях Rivna. Cron перевіряє
...
Адміністратор простору може створити в налаштуваннях одноразове запрошення
для учасника, глядача або іншого адміністратора. Посилання діє сім днів,
прив’язане до вказаної email-адреси та зберігається в базі лише як SHA-256
хеш. Після входу запрошений користувач відкриває посилання й автоматично
приєднується до спільного фінансового простору.

## CSV

Підтримується імпорт до 2 000 рядків за один файл і до 5 МБ. Очікувані колонки:
`Назва, Категорія, Дата, Сума`. Експорт із Rivna створює сумісний формат.

Ці самі колонки підтримуються у файлах Microsoft Excel `.xlsx`. У розділі
операцій доступні окремі кнопки експорту CSV та Excel, а в налаштуваннях —
імпорт обох форматів.
