...
import {ArrowLeft,ArrowRight,Check,CircleDollarSign,CreditCard,Goal,HeartHandshake,LayoutGrid,Sparkles,ShoppingCart,Coffee,House,Bus,Car,HeartPulse,Sparkles as Beauty,Shirt,Gamepad2,Repeat2,Gift,Wifi,GraduationCap,Plane,Dumbbell,Wallet,PawPrint,Smartphone,CircleDollarSign as Other,Calendar,CalendarDays,CalendarRange} from "lucide-react";
...
{step===2&&<div className="wizard-step centered"><span className="wizard-icon"><LayoutGrid/></span><small>Період планування</small><h1>Як зручніше планувати?</h1><p>Rivna адаптує прогнози та ліміти під ваш ритм.</p><div className="choice-cards period-choice-cards">
  <button className={period==="month"?"selected":""} onClick={()=>setPeriod("month")}><span className="period-choice-icon"><Calendar/></span><strong>Місяць</strong><small>Класичний бюджет від зарплати до зарплати</small></button>
  <button className={period==="week"?"selected":""} onClick={()=>setPeriod("week")}><span className="period-choice-icon"><CalendarDays/></span><strong>Тиждень</strong><small>Короткі цикли та швидший контроль</small></button>
  <button className={period==="both"?"selected":""} onClick={()=>setPeriod("both")}><span className="period-choice-icon"><CalendarRange/></span><strong>Місяць і тиждень</strong><small>Для максимальної гнучкості</small><em className="period-choice-badge">Рекомендуємо</em></button>
</div></div>}{step===3&&<div className="wizard-step categories-step"><span className="wizard-icon"><LayoutGrid/></span><small>Категорії та ліміти</small><h1>Оберіть важливе</h1><p>Увімкніть потрібні категорії та за бажанням одразу задайте ліміт.</p><div className="onboarding-categories">{(showAllCategories?categories:categories.slice(0,PRIMARY_CATEGORY_COUNT)).map((item)=>{const index=categories.indexOf(item);const CatIcon=categoryIcons[index]||CircleDollarSign;return <div key={item.name} className={item.selected?"selected":""}><button type="button" onClick={()=>setCategories(values=>values.map((value,i)=>i===index?{...value,selected:!value.selected}:value))}><i style={{background:item.color}}>{item.selected?<Check size={13}/>:<CatIcon size={13}/>}</i><span>{item.name}</span></button>        <div className="limit-inputs">
