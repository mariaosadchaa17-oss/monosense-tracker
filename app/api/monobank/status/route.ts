...
export async function GET() {
    ...
    const { data: connection } = await admin.from("monobank_connections").select("accounts_json").eq("household_id", context.householdId).maybeSingle();
    ...
    return NextResponse.json({ connected: true, accounts: connection.accounts_json || [], links: linksMap });
}