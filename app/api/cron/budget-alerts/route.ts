...
export async function GET(request:Request){
  ...
  for(const alert of alerts||[]){
    ...
    const payload=JSON.stringify({title:threshold===100?"Ліміт вичерпано":"Бюджет майже вичерпано",body:`${alert.category_name}: ${Math.round(percent)}% місячного ліміту`,url:"/?section=budget",tag:`budget-${alert.id}-${threshold}`});
