...
import {
  ArrowDownLeft, ArrowRight, ArrowUpRight, BarChart3, Bell,
...
export function FinoraApp({ initialLoggedIn = false }: { initialLoggedIn?: boolean }) {
  ...
  const [dark, setDark] = useState(()=>typeof window!=="undefined"&&localStorage.getItem("finora-theme")==="dark");
...
function AnalyticsView({transactions}:{transactions:Transaction[]}) {
  const expenses=transactions.filter(t=>t.amount<0),total=expenses.reduce((s,t)=>s+Math.abs(t.amount),0);
  const income=transactions.filter(t=>t.amount>0).reduce((s,t)=>s+t.amount,0),impulsive=expenses.filter(t=>t.impulse).reduce((s,t)=>s+Math.abs(t.amount),0);
  const grouped=Object.entries(expenses.reduce<Record<string,number>>((acc,t)=>{acc[t.category]=(acc[t.category]||0)+Math.abs(t.amount);return acc;},{})).sort((a,b)=>b[1]-a[1]);
  const now=new Date(),months=Array.from({length:6},(_,offset)=>{const date=new Date(now.getFullYear(),now.getMonth()-5+offset,1);const key=`${date.getFullYear()}-${String(date.getMonth()+1).padStart(2,"0")}`;return {key,label:new Intl.DateTimeFormat("uk-UA",{month:"short"}).format(date).replace(".",""),value:0};});
  const byMonth=new Map(months.map(m=>[m.key,m]));expenses.forEach(t=>{if(!t.bookedAt)return;const date=new Date(t.bookedAt),key=`${date.getFullYear()}-${String(date.getMonth()+1).padStart(2,"0")}`;const month=byMonth.get(key);if(month)month.value+=Math.abs(t.amount);});
  const current=months.at(-1)?.value||0,previous=months.at(-2)?.value||0,delta=previous?Math.round((current-previous)/previous*100):0,maxMonth=Math.max(...months.map(m=>m.value),1);
  return <><div className="metric-grid"><article className="metric"><small>Усі витрати</small><strong>₴ {formatMoney(total)}</strong><span>Поточний період</span></article><article className="metric"><small>Доходи</small><strong>₴ {formatMoney(income)}</strong><span className="positive">Чистий потік ₴ {formatMoney(income-total)}</span></article><article className="metric"><small>Імпульсивні покупки</small><strong>₴ {formatMoney(impulsive)}</strong><span>{total?Math.round(impulsive/total*100):0}% усіх витрат</span></article></div>
    <section className="panel monthly-panel"><div className="section-title"><div><h2>Динаміка витрат</h2><p>Останні шість місяців</p></div><span className={delta>0?"comparison negative":"comparison positive"}>{previous?`${delta>0?"+":""}${delta}% до минулого місяця`:"Ще немає порівняння"}</span></div><div className="monthly-chart">{months.map(m=><div key={m.key}><strong>{m.value?`₴${formatMoney(m.value)}`:"—"}</strong><span><i style={{height:`${Math.max(m.value?8:2,m.value/maxMonth*100)}%`}}/></span><small>{m.label}</small></div>)}</div></section>
...
function ExpenseModal({amount,setAmount,note,setNote,accounts,categories,submit,close}:{amount:string;setAmount:(s:string)=>void;note:string;setNote:(s:string)=>void;accounts:Account[];categories:CategoryItem[];submit:(e:React.FormEvent<HTMLFormElement>)=>void;close:()=>void}) { return <div className="modal-backdrop" onMouseDown={close}><form className="expense-modal tall-modal" onSubmit={submit} onMouseDown={e=>e.stopPropagation()}><ModalHead label="Деталізація операції" title="Нова витрата" close={close}/><label className="amount-field"><span>₴</span><input autoFocus inputMode="decimal" placeholder="0" value={amount} onChange={e=>setAmount(e.target.value)}/></label><div className="form-two"><label>Рахунок<select name="account">{accounts.map(a=><option key={a.id} value={a.id}>{a.name} · {a.currency}</option>)}</select></label><label>Категорія<select name="category"><option value="">Без категорії</option>{categories.filter(c=>c.kind==="expense").map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label></div><label>Дата<input name="date" type="datetime-local"/></label><label>Нотатка<input placeholder="Наприклад, кава" value={note} onChange={e=>setNote(e.target.value)}/></label><label>Теги<input name="tags" placeholder="#відпустка #робота"/></label><details className="split-details"><summary>Розділити чек</summary><div className="form-two"><label>Загальна сума<input name="splitTotal" type="number" min="0" step=".01"/></label><label>Моя частка<input name="personalShare" type="number" min="0" step=".01"/></label></div><label>Учасники<input name="splitParticipants" placeholder="Діма, Оля, Андрій"/></label><small className="field-help">Залишок буде порівну розподілений між учасниками, а з балансу спишеться лише ваша частка.</small></details><label className="check impulse"><input name="impulse" type="checkbox"/> Імпульсивна витрата</label><button className="primary">Додати витрату</button></form></div>; }
