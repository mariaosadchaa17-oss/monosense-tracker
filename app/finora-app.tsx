...
export function FinoraApp({ initialLoggedIn = false }: { initialLoggedIn?: boolean }) {
  ...
  const [modal, setModal] = useState<"expense" | "account" | "goal" | "debt" | "recurring" | "transfer" | "budget" | "category" | null>(null);
  ...
  return <main className="app-shell">
    ...
    <section className="content">
      ...
      {page === "Налаштування" && <SettingsView dark={dark} setDark={setDark} importCsv={importCsv} categories={categories} audit={audit} addCategory={()=>setModal("category")} deleteCategory={id=>financeAction({action:"deleteCategory",id},"Категорію видалено")} logout={async () => {
        if (initialLoggedIn) {
