...
export function FinoraApp({ initialLoggedIn = false }: { initialLoggedIn?: boolean }) {
  ...
  const balance = useMemo(() => accounts.reduce((sum,a)=>{if(a.currency==="UAH")return sum+a.balance;const custom=customRates.find(rate=>rate.currency===a.currency),official=rates.find(rate=>rate.currency===a.currency);return sum+a.balance*(custom?.rate||official?.rate||1)},0),[accounts,rates,customRates]);
  ...
  return <main className="app-shell">
    ...
    <section className="content">
      ...
      {page === "Головна" && <Dashboard balance={balance} accounts={accounts} transactions={transactions} goals={goals} openPage={setPage} addAccount={() => setModal("account")}/>}
      ...
      {page === "Бюджет" && (initialLoggedIn?<LiveBudgetView budgets={savedBudgets} transactions={transactions} period={planningPeriod} add={()=>setModal("budget")}/>:<BudgetView budgets={savedBudgets} transactions={transactions} add={()=>setModal("budget")}/>)}
    ...
    {modal === "budget" && <BudgetModal categories={categories} period={planningPeriod} submit={addBudget} close={()=>setModal(null)}/>}
...
function Dashboard({ balance, accounts, transactions, goals, openPage, addAccount }: {balance:number;accounts:Account[];transactions:Transaction[];goals:GoalItem[];openPage:(p:Page)=>void;addAccount:()=>void}) {
  ...
  const income=currentTransactions.filter(transaction=>transaction.amount>0).reduce((sum,transaction)=>sum+transaction.amount,0),expense=currentTransactions.filter(transaction=>transaction.amount<0).reduce((sum,transaction)=>sum+Math.abs(transaction.amount),0);
  const previousExpense=previousTransactions.filter(transaction=>transaction.amount<0).reduce((sum,transaction)=>sum+Math.abs(transaction.amount),0),difference=previousExpense?Math.round((expense-previousExpense)/previousExpense*100):0;
  ...
  return <><div className="summary-grid"><article className="balance-card"><div className="card-top"><span>Загальний баланс</span><button onClick={()=>openPage("Рахунки")}>UAH <ChevronDown/></button></div><h2>₴ {formatMoney(balance)}<small>.00</small></h2><div className="balance-meta"><span><ArrowUpRight/> +₴ {formatMoney(income)} <small>доходи</small></span><span><ArrowDownLeft/> −₴ {formatMoney(expense)} <small>витрати</small></span></div><div className="balance-footer"><span>За {monthLabel}</span><span className={difference>0?"negative":"positive"}>{previousExpense?`${difference>0?"+":""}${difference}% до минулого місяця`:"Перший період"}</span></div></article>
    ...
    <div className="dashboard-grid"><section className="panel transactions"><div className="section-title"><div><h2>Останні операції</h2><p>Найновіші записи</p></div><button onClick={() => openPage("Операції")}>Усі операції <ArrowRight/></button></div><TransactionList transactions={transactions.slice(0,4)}/></section><section className="panel budget-panel"><div className="section-title"><div><h2>Бюджет: {monthLabel}</h2><p>{daysInMonth-now.getDate()} днів до кінця місяця</p></div><button onClick={() => openPage("Бюджет")}><MoreHorizontal/></button></div><div className="budget-total"><div><small>Витрачено цього місяця</small><strong>₴ {formatMoney(expense)}</strong></div><b>{forecast?Math.round(expense/forecast*100):0}% часу</b></div><div className="main-progress"><i style={{width:`${Math.min(100,now.getDate()/daysInMonth*100)}%`}}/></div><button className="budget-open" onClick={()=>openPage("Бюджет")}>Переглянути ліміти категорій <ArrowRight/></button></section></div>
...
function LiveBudgetView({budgets,transactions,period,add}:{budgets:BudgetItem[];transactions:Transaction[];period:"month"|"week";add:()=>void}){
  const [renderedAt]=useState(()=>Date.now()),now=new Date(renderedAt),weekStart=new Date(now);
  weekStart.setHours(0,0,0,0);weekStart.setDate(weekStart.getDate()-((weekStart.getDay()+6)%7));
  ...
  const active=budgets.filter(budget=>period==="week"?budget.month.slice(0,10)===periodKey:budget.month.startsWith(periodKey));
  const periodTransactions=transactions.filter(transaction=>{if(!transaction.bookedAt)return true;const date=new Date(transaction.bookedAt);return date>=(period==="week"?weekStart:new Date(now.getFullYear(),now.getMonth(),1))&&date<periodEnd});
  const spentBy=periodTransactions.filter(transaction=>transaction.amount<0).reduce<Record<string,number>>((sum,transaction)=>{sum[transaction.category]=(sum[transaction.category]||0)+Math.abs(transaction.amount);return sum},{});
  const plan=active.reduce((sum,budget)=>sum+budget.limit,0),spent=Object.values(spentBy).reduce((sum,value)=>sum+value,0);
  const totalDays=period==="week"?7:new Date(now.getFullYear(),now.getMonth()+1,0).getDate(),elapsedDays=period==="week"?Math.min(7,Math.floor((now.getTime()-weekStart.getTime())/86400000)+1):now.getDate(),forecast=Math.round(spent/Math.max(1,elapsedDays)*totalDays);
  const periodLabel=period==="week"?"тиждень":"місяць",planLabel=period==="week"?"Тижневий план":"Місячний план";
  return <><div className="period-note"><Repeat2/> Формат планування: <strong>{period==="week"?"щотижня":"щомісяця"}</strong></div><div className="metric-grid"><article className="metric"><small>{planLabel}</small><strong>₴ {formatMoney(plan)}</strong><span>{plan?Math.round(spent/plan*100):0}% використано</span></article><article className="metric"><small>Прогноз витрат</small><strong>₴ {formatMoney(forecast)}</strong><span className={plan&&forecast>plan?"negative":"positive"}>{!plan?"Додайте перший ліміт":forecast>plan?"Можливий перерозхід":"У межах плану"}</span></article><article className="metric"><small>Очікувана економія</small><strong>₴ {formatMoney(Math.max(0,plan-forecast))}</strong><span>За поточного темпу</span></article></div><section className="panel full-view"><div className="section-title"><div><h2>Ліміти за категоріями</h2><p>Поточний {periodLabel}</p></div><button className="small-primary" onClick={add}><Plus/> Додати ліміт</button></div>{active.length?<div className="large-budget"><div className="budget-list">{active.map(budget=>{const used=spentBy[budget.name]||0,percent=Math.round(used/budget.limit*100);return <div className="budget-item" key={budget.id}><span className="budget-icon" style={{color:budget.color,background:`${budget.color}15`}}><Utensils/></span><div><div><strong>{budget.name}</strong><small>₴ {formatMoney(used)} / {formatMoney(budget.limit)} · {percent}%</small></div><span><i style={{width:`${Math.min(100,percent)}%`,background:percent>=100?"#e05252":percent>=80?"#f4b740":budget.color}}/></span></div></div>})}</div></div>:<div className="empty"><p>Лімітів на цей {periodLabel} ще немає.</p><button className="small-primary" onClick={add}><Plus/> Створити перший ліміт</button></div>}{active.some(budget=>(spentBy[budget.name]||0)/budget.limit>=.8)&&<div className="alert-card"><Bell/><div><strong>Наближення до ліміту</strong><p>Одна або кілька категорій використані більш ніж на 80%.</p></div></div>}</section></>
}
...
function AnalyticsView({transactions}:{transactions:Transaction[]}) {
  ...
  transactions.filter(transaction=>transaction.amount<0).forEach(transaction=>{const bucket=bucketFor(transaction);if(bucket)bucket.value+=Math.abs(transaction.amount)});
  const currentTransactions=transactions.filter(transaction=>bucketFor(transaction)===buckets.at(-1)),expenses=currentTransactions.filter(transaction=>transaction.amount<0),total=expenses.reduce((sum,transaction)=>sum+Math.abs(transaction.amount),0),income=currentTransactions.filter(transaction=>transaction.amount>0).reduce((sum,transaction)=>sum+transaction.amount,0),impulsive=expenses.filter(transaction=>transaction.impulse).reduce((sum,transaction)=>sum+Math.abs(transaction.amount),0);
  const grouped=Object.entries(expenses.reduce<Record<string,number>>((sum,transaction)=>{sum[transaction.category]=(sum[transaction.category]||0)+Math.abs(transaction.amount);return sum},{})).sort((a,b)=>b[1]-a[1]);
  const current=buckets.at(-1)?.value||0,previous=buckets.at(-2)?.value||0,delta=previous?Math.round((current-previous)/previous*100):0,maxValue=Math.max(...buckets.map(bucket=>bucket.value),1);
  return <><div className="period-switch"><button className={period==="month"?"active":""} onClick={()=>setPeriod("month")}>За місяцями</button><button className={period==="week"?"active":""} onClick={()=>setPeriod("week")}>За тижнями</button></div><div className="metric-grid"><article className="metric"><small>Витрати за {period==="month"?"місяць":"тиждень"}</small><strong>₴ {formatMoney(total)}</strong><span>Поточний період</span></article><article className="metric"><small>Доходи</small><strong>₴ {formatMoney(income)}</strong><span className="positive">Чистий потік ₴ {formatMoney(income-total)}</span></article><article className="metric"><small>Імпульсивні покупки</small><strong>₴ {formatMoney(impulsive)}</strong><span>{total?Math.round(impulsive/total*100):0}% усіх витрат</span></article></div>
    <section className="panel monthly-panel"><div className="section-title"><div><h2>Динаміка витрат</h2><p>{period==="month"?"Останні шість місяців":"Останні вісім тижнів"}</p></div><span className={delta>0?"comparison negative":"comparison positive"}>{previous?`${delta>0?"+":""}${delta}% до попереднього періоду`:"Ще немає порівняння"}</span></div><div className="monthly-chart">{buckets.map(bucket=><div key={bucket.key}><strong>{bucket.value?`₴${formatMoney(bucket.value)}`:"—"}</strong><span><i style={{height:`${Math.max(bucket.value?8:2,bucket.value/maxValue*100)}%`}}/></span><small>{bucket.label}</small></div>)}</div></section>
...
function ProfileSettings({dark,setDark,notify}:{dark:boolean;setDark:(value:boolean)=>void;notify:(message:string)=>void}){
  ...
  async function save(e:React.FormEvent<HTMLFormElement>){e.preventDefault();if(!profile)return;const response=await fetch("/api/settings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"saveProfile",...profile})});const result=await response.json();notify(response.ok?"Налаштування збережено":result.error||"Не вдалося зберегти");}
  return <form className="panel settings-card" onSubmit={save}><h2>Загальні</h2><label>Ваше ім’я<input value={profile?.name||""} onChange={e=>setProfile(p=>p?{...p,name:e.target.value}:p)} placeholder="Ваше ім’я"/></label><label>Базова валюта<select value={profile?.baseCurrency||"UAH"} onChange={e=>setProfile(p=>p?{...p,baseCurrency:e.target.value}:p)}><option>UAH</option><option>USD</option><option>EUR</option><option>GBP</option><option>PLN</option></select></label><label>Період планування<select value={profile?.planningPeriod||"month"} onChange={e=>setProfile(p=>p?{...p,planningPeriod:e.target.value==="week"?"week":"month"}:p)}><option value="month">Місяць</option><option value="week">Тиждень</option></select></label><label>Telegram chat ID<input value={profile?.telegramChatId||""} onChange={e=>setProfile(p=>p?{...p,telegramChatId:e.target.value}:p)} placeholder="Надішліть боту /start"/></label><label className="setting-toggle"><span><strong>Нагадування про платежі</strong><small>Для неавтоматичних правил</small></span><input type="checkbox" checked={profile?.recurringReminders??true} onChange={e=>setProfile(p=>p?{...p,recurringReminders:e.target.checked}:p)}/></label><label className="setting-toggle"><span><strong>Темна тема</strong><small>Змінити вигляд застосунку</small></span><input type="checkbox" checked={dark} onChange={e=>setDark(e.target.checked)}/></label><button className="primary" disabled={!profile}>Зберегти</button></form>;
}
type SharedMember={userId:string;name:string;role:string;joinedAt:string;isMe:boolean};
type PendingInvite={id:string;email:string;role:string;expires_at:string};
function MembersPanel({notify}:{notify:(message:string)=>void}){
  const [members,setMembers]=useState<SharedMember[]>([]),[invites,setInvites]=useState<PendingInvite[]>([]),[myRole,setMyRole]=useState("");
  async function load(){const response=await fetch("/api/settings",{cache:"no-store"});if(!response.ok)return;const data=await response.json();setMembers(data.members||[]);setInvites(data.invitations||[]);setMyRole(data.profile?.role||"");}
...
function AccountCard({account}:{account:Account}) { const [renderedAt]=useState(()=>Date.now()),days=account.graceEnd?Math.ceil((new Date(account.graceEnd).getTime()-renderedAt)/86400000):null;return <article className={`account ${bankStyle(account.bank)}`}><div><BankMark bank={account.bank}/>{days!==null&&<em className={days<=7?"grace urgent":"grace"}>{days>=0?`${days} дн. грейсу`:"Грейс минув"}</em>}</div><p>{account.name}</p><h3>{account.currency==="UAH"?"₴":account.currency==="EUR"?"€":"$"} {formatMoney(account.balance)}</h3><small>{account.bank} · {account.owner}{account.creditLimit?` · ліміт ${formatMoney(account.creditLimit)}`:""}</small></article>; }
...
function ExpenseModal({amount,setAmount,note,setNote,accounts,categories,submit,close}:{amount:string;setAmount:(s:string)=>void;note:string;setNote:(s:string)=>void;accounts:Account[];categories:CategoryItem[];submit:(e:React.FormEvent<HTMLFormElement>)=>void;close:()=>void}) {
  const accountOptions=accounts.map(account=>({value:String(account.id),label:`${account.name} · ${account.currency}`})),categoryOptions=[{value:"",label:"Без категорії"},...categories.filter(category=>category.kind==="expense").map(category=>({value:category.id,label:category.name}))];
  return <div className="modal-backdrop" onMouseDown={close}><form className="expense-modal tall-modal" onSubmit={submit} onMouseDown={event=>event.stopPropagation()}><ModalHead label="Деталізація операції" title="Нова витрата" close={close}/><label className="amount-field"><span>₴</span><input autoFocus inputMode="decimal" placeholder="0" value={amount} onChange={event=>setAmount(event.target.value)}/></label><div className="form-two"><WheelField name="account" label="Рахунок" options={accountOptions}/><WheelField name="category" label="Категорія" options={categoryOptions}/></div><DateWheelField name="date"/><label>Нотатка<input placeholder="Наприклад, кава" value={note} onChange={event=>setNote(event.target.value)}/></label><label>Теги<input name="tags" placeholder="#відпустка #робота"/></label><details className="split-details"><summary>Розділити чек</summary><div className="form-two"><label>Загальна сума<input name="splitTotal" type="number" min="0" step=".01"/></label><label>Моя частка<input name="personalShare" type="number" min="0" step=".01"/></label></div><label>Учасники<input name="splitParticipants" placeholder="Діма, Оля, Андрій"/></label><small className="field-help">Залишок буде порівну розподілений між учасниками, а з балансу спишеться лише ваша частка.</small></details><details className="split-details"><summary>Повторювати витрату</summary><label className="check impulse"><input name="repeat" type="checkbox"/> Створити регулярне нагадування</label><div className="form-two"><label>Період<select name="repeatFrequency"><option value="weekly">Щотижня</option><option value="monthly">Щомісяця</option><option value="yearly">Щороку</option></select></label><label>Число місяця<input name="repeatDay" type="number" min="1" max="28" placeholder="Наприклад, 5"/></label></div></details><label className="check impulse"><input name="impulse" type="checkbox"/> Імпульсивна витрата</label><button className="primary">Додати витрату</button></form></div>;
}
function WheelField({name,label,options}:{name:string;label:string;options:{value:string;label:string}[]}){const [value,setValue]=useState(options[0]?.value||""),selected=options.find(option=>option.value===value);return <label className="picker-label">{label}<details className="compact-picker"><summary>{selected?.label||"Оберіть"}</summary><div className="picker-wheel">{options.map(option=><button type="button" key={`${name}-${option.value}`} className={option.value===value?"selected":""} onClick={event=>{setValue(option.value);event.currentTarget.closest("details")?.removeAttribute("open")}}>{option.label}{option.value===value&&<Check/>}</button>)}</div></details><input type="hidden" name={name} value={value}/></label>}
function DateWheelField({name}:{name:string}){const [renderedAt]=useState(()=>Date.now()),options=Array.from({length:38},(_,index)=>{const date=new Date(renderedAt);date.setDate(date.getDate()+index-7);const value=date.toISOString().slice(0,10);return {value,label:new Intl.DateTimeFormat("uk-UA",{weekday:"short",day:"numeric",month:"long"}).format(date)}});return <WheelField name={name} label="Дата" options={options}/>}
function AccountModal({account,submit,close}:{account:Account|null;submit:(e:React.FormEvent<HTMLFormElement>)=>void;close:()=>void}) { const banks=["monobank","ПриватБанк","ПУМБ","Ощадбанк","Райффайзен Банк","Готівка","Інший"],selected=banks.includes(account?.bank||"")?account?.bank:"Інший";return <div className="modal-backdrop" onMouseDown={close}><form className="expense-modal" onSubmit={submit} onMouseDown={e=>e.stopPropagation()}><ModalHead label={account?"Редагування активу":"Новий актив"} title={account?"Змінити рахунок":"Додати рахунок"} close={close}/><label>Назва<input name="name" defaultValue={account?.name} placeholder="Наприклад, Зарплатна картка" required/></label><div className="form-two"><label>Банк<select name="bank" defaultValue={selected}>{banks.map(bank=><option key={bank}>{bank}</option>)}</select></label><label>Власник<input name="owner" defaultValue={account?.owner} placeholder="Мій"/></label></div><div className="form-two"><label>Валюта<select name="currency" defaultValue={account?.currency||"UAH"}><option>UAH</option><option>USD</option><option>EUR</option><option>GBP</option><option>PLN</option></select></label><label>Баланс<input name="balance" type="number" step=".01" defaultValue={account?.balance} placeholder="0"/></label></div><details className="split-details" open={Boolean(account?.creditLimit||account?.graceEnd)}><summary>Кредитна картка</summary><div className="form-two"><label>Кредитний ліміт<input name="creditLimit" type="number" min="0" defaultValue={account?.creditLimit} placeholder="0"/></label><label>Кінець грейс-періоду<input name="graceEnd" type="date" defaultValue={account?.graceEnd?.slice(0,10)}/></label></div></details><button className="primary">{account?"Зберегти зміни":"Створити рахунок"}</button></form></div>; }
...
function BudgetModal({categories,period,submit,close}:{categories:CategoryItem[];period:"month"|"week";submit:(e:React.FormEvent<HTMLFormElement>)=>void;close:()=>void}) { const now=new Date(),weekStart=new Date(now);weekStart.setDate(weekStart.getDate()-((weekStart.getDay()+6)%7));const value=period==="week"?weekStart.toISOString().slice(0,10):`${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,"0")}`;return <div className="modal-backdrop" onMouseDown={close}><form className="expense-modal" onSubmit={submit} onMouseDown={e=>e.stopPropagation()}><ModalHead label="Планування" title="Ліміт категорії" close={close}/><label>Категорія<select name="category" required>{categories.filter(c=>c.kind==="expense").map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label><div className="form-two"><label>{period==="week"?"Перший день тижня":"Місяць"}<input name="period" type={period==="week"?"date":"month"} defaultValue={value} required/></label><label>Ліміт, ₴<input name="limit" type="number" min="1" required/></label></div><button className="primary">Зберегти ліміт</button></form></div>; }
