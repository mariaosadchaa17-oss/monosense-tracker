...
function AccountModal({submit,close}:{submit:(e:React.FormEvent<HTMLFormElement>)=>void;close:()=>void}) { return <div className="modal-backdrop" onMouseDown={close}><form className="expense-modal" onSubmit={submit} onMouseDown={e=>e.stopPropagation()}><ModalHead label="Новий актив" title="Додати рахунок" close={close}/><label>Назва<input name="name" placeholder="Наприклад, Зарплатна картка" required/></label><div className="form-two"><label>Банк<select name="bank"><option>monobank</option><option>ПриватБанк</option><option>ПУМБ</option><option>Ощадбанк</option><option>Райффайзен Банк</option><option>Готівка</option><option>Інший</option></select></label><label>Власник<input name="owner" placeholder="Мій"/></label></div><div className="form-two"><label>Валюта<select name="currency"><option>UAH</option><option>USD</option><option>EUR</option><option>GBP</option><option>PLN</option></select></label><label>Баланс<input name="balance" type="number" step=".01" placeholder="0"/></label></div><details className="split-details"><summary>Кредитна картка</summary><div className="form-two"><label>Кредитний ліміт<input name="creditLimit" type="number" min="0" placeholder="0"/></label><label>Кінець грейс-періоду<input name="graceEnd" type="date"/></label></div></details><button className="primary">Створити рахунок</button></form></div>; }
