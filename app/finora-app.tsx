...
export function FinoraApp({ initialLoggedIn = false }: { initialLoggedIn?: boolean }) {
  ...
  return <main className="app-shell">
    ...
    <section className="content">
      ...
      {page === "Бюджет" && (initialLoggedIn?<LiveBudgetView budgets={savedBudgets} transactions={transactions} add={()=>setModal("budget")}/>:<BudgetView budgets={savedBudgets} transactions={transactions} add={()=>setModal("budget")}/>)}
...
function LiveBudgetView({budgets,transactions,add}:{budgets:BudgetItem[];transactions:Transaction[];add:()=>void}){
  const [renderedAt]=useState(()=>Date.now()),now=new Date(renderedAt),monthKey=`${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,"0")}`;
  const active=budgets.filter(budget=>budget.month.startsWith(monthKey)),monthTransactions=transactions.filter(transaction=>!transaction.bookedAt||String(transaction.bookedAt).startsWith(monthKey));
  const spentBy=monthTransactions.filter(transaction=>transaction.amount<0).reduce<Record<string,number>>((sum,transaction)=>{sum[transaction.category]=(sum[transaction.category]||0)+Math.abs(transaction.amount);return sum},{});
  const plan=active.reduce((sum,budget)=>sum+budget.limit,0),spent=Object.values(spentBy).reduce((sum,value)=>sum+value,0),days=new Date(now.getFullYear(),now.getMonth()+1,0).getDate(),forecast=Math.round(spent/Math.max(1,now.getDate())*days);
  return <><div className="metric-grid"><article className="metric"><small>Місячний план</small><strong>₴ {formatMoney(plan)}</strong><span>{plan?Math.round(spent/plan*100):0}% використано</span></article><article className="metric"><small>Прогноз витрат</small><strong>₴ {formatMoney(forecast)}</strong><span className={plan&&forecast>plan?"negative":"positive"}>{!plan?"Додайте перший ліміт":forecast>plan?"Можливий перерозхід":"У межах плану"}</span></article><article className="metric"><small>Очікувана економія</small><strong>₴ {formatMoney(Math.max(0,plan-forecast))}</strong><span>За поточного темпу</span></article></div><section className="panel full-view"><div className="section-title"><div><h2>Ліміти за категоріями</h2><p>Поточний місяць</p></div><button className="small-primary" onClick={add}><Plus/> Додати ліміт</button></div>{active.length?<div className="large-budget"><div className="budget-list">{active.map(budget=>{const used=spentBy[budget.name]||0,percent=Math.round(used/budget.limit*100);return <div className="budget-item" key={budget.id}><span className="budget-icon" style={{color:budget.color,background:`${budget.color}15`}}><Utensils/></span><div><div><strong>{budget.name}</strong><small>₴ {formatMoney(used)} / {formatMoney(budget.limit)} · {percent}%</small></div><span><i style={{width:`${Math.min(100,percent)}%`,background:percent>=100?"#e05252":percent>=80?"#f4b740":budget.color}}/></span></div></div>})}</div></div>:<div className="empty"><p>Лімітів на цей місяць ще немає.</p><button className="small-primary" onClick={add}><Plus/> Створити перший ліміт</button></div>}{active.some(budget=>(spentBy[budget.name]||0)/budget.limit>=.8)&&<div className="alert-card"><Bell/><div><strong>Наближення до ліміту</strong><p>Одна або кілька категорій використані більш ніж на 80%.</p></div></div>}</section></>
}
...
type SettingsProfile={name:string;email:string;baseCurrency:string;householdName:string;telegramChatId:string;recurringReminders:boolean;budget80:boolean;budget100:boolean;role:string};
function ProfileSettings({dark,setDark,notify}:{dark:boolean;setDark:(value:boolean)=>void;notify:(message:string)=>void}){
  const [profile,setProfile]=useState<SettingsProfile|null>({name:"Марія",email:"",baseCurrency:"UAH",householdName:"Мої фінанси",telegramChatId:"",recurringReminders:true,budget80:true,budget100:true,role:"owner"});
  ...
  return <form className="panel settings-card" onSubmit={save}><h2>Загальні</h2><label>Ваше ім’я<input value={profile?.name||""} onChange={e=>setProfile(p=>p?{...p,name:e.target.value}:p)} placeholder="Ваше ім’я"/></label><label>Базова валюта<select value={profile?.baseCurrency||"UAH"} onChange={e=>setProfile(p=>p?{...p,baseCurrency:e.target.value}:p)}><option>UAH</option><option>USD</option><option>EUR</option><option>GBP</option><option>PLN</option></select></label><label>Telegram chat ID<input value={profile?.telegramChatId||""} onChange={e=>setProfile(p=>p?{...p,telegramChatId:e.target.value}:p)} placeholder="Надішліть боту /start"/></label><label className="setting-toggle"><span><strong>Нагадування про платежі</strong><small>Для неавтоматичних правил</small></span><input type="checkbox" checked={profile?.recurringReminders??true} onChange={e=>setProfile(p=>p?{...p,recurringReminders:e.target.checked}:p)}/></label><label className="setting-toggle"><span><strong>Темна тема</strong><small>Змінити вигляд застосунку</small></span><input type="checkbox" checked={dark} onChange={e=>setDark(e.target.checked)}/></label><button className="primary" disabled={!profile}>Зберегти</button></form>;
}
