...
export function RivnaApp({ initialLoggedIn = false }: { initialLoggedIn?: boolean }) {
  ...
  const balance=useMemo(()=>accounts.reduce((sum,account)=>sum+Math.max(0,account.balance)*conversionRate(account.currency,rates,customRates)/conversionRate(baseCurrency,rates,customRates),0),[accounts,rates,customRates,baseCurrency]);
  ...
  const allDebts=useMemo(()=>{
    const creditDebts=accounts.filter(a=>a.creditLimit>0&&a.balance<0).map(a=>({id:`credit-${a.id}`,person:a.bank,direction:"i_owe" as const,amount:Math.abs(a.balance),currency:a.currency,note:"Кредитні кошти",isVirtual:true}));
...
function AccountCard({account}:{account:Account}) {
  ...
  return <article className={`account ${bankStyle(account.bank)}`} style={account.color?{background:account.color}:undefined}>
    ...
    <div style={{fontSize: '10px', color: 'rgba(255,255,255,.7)', marginTop: '4px', display: 'grid', gap: '4px'}}>
      <span>
        {account.balance >= 0 
    ...
    <small style={{marginTop: '10px'}}>{account.bank} · {account.owner}</small>
  </article>;
}
