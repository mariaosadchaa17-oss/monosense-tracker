...
type DebtItem = {id:string;person:string;direction:"owed_to_me"|"i_owe";amount:number;currency:string;due?:string;note?:string;isVirtual?:boolean};
...
export function RivnaApp({ initialLoggedIn = false }: { initialLoggedIn?: boolean }) {
  ...
  const allDebts=useMemo(()=>{
    const creditDebts=accounts.filter(a=>(a.creditLimit||0)>0&&a.balance<0).map(a=>({id:`credit-${a.id}`,person:a.bank,direction:"i_owe" as const,amount:Math.abs(a.balance),currency:a.currency,note:"Кредитні кошти",isVirtual:true}));
  ...
  async function addTransfer(e:React.SyntheticEvent<HTMLFormElement>){e.preventDefault();const f=new FormData(e.currentTarget);if(await financeAction({action:"createTransfer",fromAccountId:f.get("from"),toAccountId:f.get("to"),sentAmount:Number(f.get("sent")),receivedAmount:Number(f.get("received")),exchangeRate:Number(f.get("rate")),feeAmount:Number(f.get("fee")),feeCurrency:String(f.get("feeCurrency")),note:String(f.get("note")||"")},"Переказ виконано"))setModal(null);}
  async function addBudget(e:React.SyntheticEvent<HTMLFormElement>){e.preventDefault();const f=new FormData(e.currentTarget);const raw=String(f.get("period"));if(await financeAction({action:"createBudget",categoryId:f.get("category"),month:planningPeriod==="week"?raw:`${raw}-01`,periodType:planningPeriod,limitAmount:Number(f.get("limit")),currency:baseCurrency},"Ліміт збережено"))setModal(null);}
  ...
  return <main className="app-shell">
    ...
    <section className="content">
      ...
      {page === "Бюджет" && (initialLoggedIn?<LiveBudgetView budgets={savedBudgets} transactions={normalizedTransactions} period={planningPeriod} baseCurrency={baseCurrency} add={()=>setModal("budget")}/>:<BudgetView budgets={savedBudgets} transactions={normalizedTransactions} baseCurrency={baseCurrency} add={()=>setModal("budget")}/>)}
      ...
      {page === "Накопичення" && <GoalsView goals={goals} add={()=>setModal("goal")} contribute={(id,amount)=>financeAction({action:"contributeGoal",id,amount},"Ціль поповнено")} recurring={recurring} addRecurring={()=>setModal("recurring")} remove={id=>financeAction({action:"deleteGoal",id},"Ціль видалено")}/>}
