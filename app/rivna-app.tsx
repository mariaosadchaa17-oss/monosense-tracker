...
async function addDebt(e:React.SyntheticEvent<HTMLFormElement>){
  ...
  useEffect(()=>{
    if(!initialLoggedIn)return;
    fetch("/api/monobank/status").then(r=>r.ok?r.json():null).then(data=>{
      if(data?.connected){setMonoAccounts(data.accounts||[]);setMonoLinks(data.links||{})}
  ...
  async function resyncMonobank(){
    ...
    try{
      ...
      let result:{error?:string;imported?:number;debug?:{monoAccountId:string;status?:number;error?:string;itemsFound?:number}[]}={};
      ...
      notify(failed?`Завантажено: ${result.imported ?? 0}. Проблема: ${failed.error} (${failed.status||"немає з'єднання"})`:`Завантажено операцій: ${result.imported ?? 0}`);
  ...
  <section className="panel mono-panel"><div className="section-title"><div><h2>Monobank</h2><p>Автоматичне вивантаження виписки в реальному часі</p></div><div className="title-actions">{monoAccounts.length>0 && <button className="secondary" onClick={resyncMonobank} disabled={monoResyncing}>{monoResyncing?"Оновлюю…":"Оновити за 31 день"}</button>}<button onClick={()=>setMonoOpen(v=>!v)}>{monoOpen?"Згорнути":"Підключити"}</button></div></div>{monoAccounts.length===0&&<p className="mono-note" style={{padding:"0 0 12px"}}>Спочатку підключи токен нижче — після цього зʼявиться кнопка оновлення виписки.</p>}
    {monoOpen && <>
      ...
      {monoAccounts.length>0 && <div className="mono-accounts-list">{monoAccounts.map(ma=>{
        ...
        return <div key={ma.id} className="mono-account-row">
          <div><strong>{ma.type==="fop"?"ФОП":ma.type==="jar"?"Банка":"Картка"} {ma.maskedPan}</strong><small>{ma.currency} · {ma.balance.toFixed(0)}</small></div>
          {linkedAccount ? <span className="mono-linked-badge">✓ Прив'язано: {linkedAccount.name}</span> : <select defaultValue="" onChange={e=>{if(e.target.value==="__new__")createAndLinkMonobankAccount(ma);else linkMonobankAccount(ma.id,e.target.value)}}>
