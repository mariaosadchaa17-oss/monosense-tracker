...
export function RivnaApp({ initialLoggedIn = false }: { initialLoggedIn?: boolean }) {
  ...
  async function addCustomRate(e:React.FormEvent<HTMLFormElement>){e.preventDefault();const f=new FormData(e.currentTarget);if(await financeAction({action:"createCustomRate",quoteCurrency:f.get("currency"),rate:Number(f.get("rate")),date:String(f.get("date")||new Date().toISOString().slice(0,10))},"Власний курс збережено"))setModal(null)}
...
function LiveBudgetView({budgets,transactions,period,baseCurrency,add}:{budgets:BudgetItem[];transactions:Transaction[];period:"month"|"week";baseCurrency:string;add:()=>void}){
  ...
  return <>
    ...
    <section className="panel full-view">
      ...
      })}</div></div>:<div className="empty"><p>Лімітів на цей {periodLabel} еще нет.</p><button className="small-primary" onClick={add}><Plus/> Створити перший ліміт</button></div>}
    ...
    <div style={{fontSize: '10px', color: isLight(account.color) ? 'rgba(0,0,0,.7)' : 'rgba(255,255,255,.7)', marginTop: '4px', display: 'grid', gap: '4px'}}>
      <span>
        {account.balance >= 0 
          ...
          : `Використано: ${formatMoney(account.balance)} з ${formatMoney(account.creditLimit)}`
        }
      </span>
    </div>
